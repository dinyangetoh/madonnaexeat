 ...
 * @author Rodrigo Coelho <rodrigo@giix.org>
 */
abstract class GxActiveRecord extends EActiveRecord {
	...
	public function beforeValidate(){
		...
		if($this->isNewRecord){
			// set the create date, last updated date and the user doing the creating
                        if($this->hasAttribute('created')) // if model have createdDate Field
                        $this->created=$this->modified=new CDbExpression('NOW()');
			if($this->hasAttribute('createdby'))
                        $this->createdby=$this->modifiedby=Yii::app()->user->id;
                       
                     if($this->hasAttribute('adminstatus')) {
		...
		else
		{
                    ...
                    if($this->hasAttribute('modifiedby'))
			$this->modifiedby=Yii::app()->user->id;
                     if(isset($this->reviewed))  { 
                        $this->reviewed=new CDbExpression('NOW()');
